...
class MigrationService:
    ...
    async def get_available_recipes(self, token: str, repo_url: str, owner: str, repo: str) -> List[Dict[str, Any]]:
        ...
        analysis = await github_service.analyze_repository(token, owner, repo, repo_url, include_deep_analysis=True)
                ...
                if file.endswith('.java'):
                    ...
                    try:
                        with open(filepath, 'r', encoding='utf-8') as f:
                            ...
                            for endpoint in self._extract_endpoints_from_java_content(content, file):
...
"""
    ...
    async def update_java_version_in_project(self, project_path: str, target_version: str) -> list[str]:
        """Update Java version in pom.xml or build.gradle files in the project directory"""
        modified_files = []
        import re

        for root, dirs, files in os.walk(project_path):
            dirs[:] = [d for d in dirs if d not in {".git", "node_modules", "target", "build", "venv"}]
            for file in files:
                if file == "pom.xml":
                    full_path = os.path.join(root, file)
                    rel_path = os.path.relpath(full_path, project_path)
                    if await self._update_maven_java_version(full_path, target_version):
                    ...
                    try:
                        ...
                        new_content = re.sub(source_pattern, f"sourceCompatibility = '{target_version}'", new_content)
                        source_pattern_num = r"sourceCompatibility\s*=\s*([0-9\.]+)"
                        new_content = re.sub(source_pattern_num, f"sourceCompatibility = {target_version}", new_content)
                        
                        # Update targetCompatibility
                        target_pattern = r"targetCompatibility\s*=\s*['\"](\d+)['\"]"
                        new_content = re.sub(target_pattern, f"targetCompatibility = '{target_version}'", new_content)
                        target_pattern_num = r"targetCompatibility\s*=\s*([0-9\.]+)"
                        new_content = re.sub(target_pattern_num, f"targetCompatibility = {target_version}", new_content)
                        ...
                        print(f"Error updating Gradle file {full_path}: {e}")
                        
        return modified_files

    async def verify_java_version_in_project(self, project_path: str, target_version: str) -> str:
        """Verify the Java version after updating it in the project files"""
        detected = await self._auto_detect_java_version(project_path)
        return detected or target_version

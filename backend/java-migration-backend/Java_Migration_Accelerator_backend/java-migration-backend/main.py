...
from services.code_complexity_service import CodeComplexityService
...
@app.get("/api/analysis/dependency-graph")
@app.get("/analysis/dependency-graph")
async def get_dependency_graph(repo_url: str, token: str = ""):
    """Analyze a repository and return its dependency graph and circular dependencies."""
    try:
        is_gitlab = "gitlab.com" in repo_url.lower()
        service = gitlab_service if is_gitlab else github_service
        
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITHUB_TOKEN
        
        # Clone repository
        clone_path = await service.clone_repository(effective_token, repo_url)
        
        # Run dependency graph analysis
        from services.dependency_graph_service import DependencyGraphService
        result = DependencyGraphService.build_graph(clone_path)
        ...
        try:
            ...
            logger.warning(f"Failed to cleanup clone directory {clone_path}: {cleanup_err}")
            
        return result
    except Exception as e:
        import traceback
        logger.error(f"Error in dependency graph analysis: {e}\n{traceback.format_exc()}")
        raise HTTPException(status_code=500, detail=str(e))



complexity_service = CodeComplexityService()
_code_complexity_cache = {}

@app.get("/api/analysis/code-complexity")
@app.get("/analysis/code-complexity")
async def analyze_code_complexity(
    repo_url: str,
    token: str = ""
):
    """
    Analyze Java code complexity for a repository.
    """

    if not repo_url.strip():
        raise HTTPException(
            status_code=400,
            detail="Missing repo_url parameter"
        )

    cached = _code_complexity_cache.get(repo_url)
    if cached:
        print(f"[CACHE HIT] Complexity {repo_url}")
        return cached
    ...
    try:
        effective_token = (
            token.strip()
            if token and token.strip()
            else DEFAULT_GITHUB_TOKEN
        )

        is_gitlab = "gitlab.com" in repo_url.lower()
        service = gitlab_service if is_gitlab else github_service

        print(f"[COMPLEXITY] Cloning {repo_url}")

        clone_path = await service.clone_repository(
            effective_token,
            repo_url
        )

        print("[COMPLEXITY] Calculating...")

        result = complexity_service.analyze(clone_path)

        _code_complexity_cache[repo_url] = result
...
async def migration_websocket(
    ...
    # connect() accepts the socket AND replays all buffered messages
...
async def get_migration_logs(job_id: str):
    ...
    if job_id not in migration_jobs:
        return {"job_id": job_id, "logs": ["[INFO] Detailed logs are no longer available for this historical session."]}
...
async def run_migration(job_id: str, request: MigrationRequest, migration_history_id: Optional[int] = None):
    ...
    try:
        ...
        await update_job(job_id, MigrationStatus.CLONING, 5, "Cloning source repository...")
    ...
    job_id,
    {
        ...
        await add_log(job_id, f"Repository cloned to {clone_path}")
        await manager.broadcast(
    job_id,
    {
        ...
        await update_job(job_id, MigrationStatus.ANALYZING, 15, "Analyzing project structure and detecting issues...")
        await manager.broadcast(
    job_id,
    {
        "type": "analysis",
        "progress": 15,
        "status": "ANALYZING",
        "message": "Analyzing project structure and detecting issues..."
    }
)
        ...
        await add_log(job_id, f"Found {job.total_errors} errors, {job.total_warnings} warnings to process")
        await manager.broadcast(
    job_id,
    {
        ...
        for conv_type in request.conversion_types:
            await update_job(job_id, MigrationStatus.MIGRATING, progress, f"Running {conv_type} migration...")
            await manager.broadcast(
    job_id,
    {
        "type": "migration",
        "progress": progress,
        "status": "MIGRATING",
        "message": f"Running {conv_type} migration..."
    }
)
            await add_log(job_id, f"Processing conversion: {conv_type}")
    ...
    job_id,
    {
        ...
        "status": "MIGRATING",
        "message": f"{conv_type} migration completed successfully!"
    }
)
        ...
        if "java_version" in request.conversion_types:
            target_version = request.target_java_version.value
            await add_log(job_id, f"Source Java Version: {request.source_java_version}")
            await add_log(job_id, f"Target Java Version: {target_version}")
            await add_log(job_id, f"Selected Conversion Types: {request.conversion_types}")
            await update_job(job_id, MigrationStatus.MIGRATING, progress, "Applying target Java version to build files...")
            await manager.broadcast(
    job_id,
    {
        ...
        "progress": progress,
        "status": "MIGRATING",
        "message": "Applying target Java version to build files..."
    }
)
            java_version_changed_files = await migration_service.update_java_version_in_project(clone_path, target_version)
            actual_version = await migration_service.verify_java_version_in_project(clone_path, target_version)
            ...
            await add_log(job_id, f"Java version verification passed: {actual_version}")
            if java_version_changed_files:
                await add_log(job_id, f"Java version changed files: {', '.join(java_version_changed_files)}")
        
        await add_log(job_id, f"Modified {job.files_modified} files, fixed {job.issues_fixed} issues")
        await manager.broadcast(
    job_id,
    {
        "type": "migration_finished",
        "progress": progress,
        "status": "MIGRATING",
        "message": "Migration step completed successfully!"
    }
)
        ...
        if request.run_tests:
            await update_job(job_id, MigrationStatus.TESTING, 60, "Running tests and validating APIs...")
            await manager.broadcast(
    job_id,
    {
        "type": "testing",
        "progress": 60,
        "status": "TESTING",
        "message": "Running tests and validating APIs..."
    }
)
            ...
            await add_log(job_id, f"Tests: {job.api_endpoints_working}/{job.api_endpoints_validated} endpoints working")
            await manager.broadcast(
    job_id,
    {
        ...
        if request.run_sonar:
            await update_job(job_id, MigrationStatus.SONAR_ANALYSIS, 75, "Running SonarQube code quality analysis...")
            await manager.broadcast(
    job_id,
    {
        "type": "sonarqube_analysis",
        "progress": 75,
        "status": "SONAR_ANALYSIS",
        "message": "Running SonarQube code quality analysis..."
            ...
            await add_log(job_id, f"SonarQube: Quality Gate = {job.sonar_quality_gate}")
            await manager.broadcast(
    job_id,
    {
        "type": "sonarqube_completed",
        "progress": 80,
        "status": "SONAR_COMPLETED",
        ...
        if getattr(request, 'run_fossa', False):
            await update_job(job_id, MigrationStatus.FOSSA_ANALYSIS, 80, "Running FOSSA license & dependency scan...")
            await manager.broadcast(
    job_id,
    {
        "type": "fossa_analysis",
        "progress": 80,
        "status": "FOSSA_ANALYSIS",
        "message": "Running FOSSA license & dependency scan..."
    }
)
            try:
                try:
                ...
                await add_log(job_id, f"FOSSA: policy={job.fossa_policy_status} deps={job.fossa_total_dependencies} vuln={job.fossa_vulnerabilities}")
                await manager.broadcast(
    job_id,
    {
        "type": "fossa_completed",
        "progress": 85,
        "status": "FOSSA_COMPLETED",
        "message": "FOSSA analysis completed successfully!"
    }
)
            except Exception as fossa_err:
                await add_log(job_id, f"FOSSA ERROR: {str(fossa_err)}")
                await manager.broadcast(
        job_id,
        {
            "type": "fossa_failed",
            "progress": 80,
            "status": "FAILED",
            "message": f"FOSSA analysis failed: {str(fossa_err)}"
        }
    )
        ...
        await add_log(job_id, f"Using GitHub token: {'user-provided' if request.token and request.token.strip() else 'default'}")
        ...
        if migration_approach == "branch":
            ...
            await add_log(job_id, f"Target branch name: {target_branch_name}")
            await update_job(job_id, MigrationStatus.PUSHING, 90, "Pushing migrated code to a new branch...")
            await manager.broadcast(
    job_id,
    {
        "type": "pushing",
        "progress": 90,
        "status": "PUSHING",
        "message": "Pushing migrated code to a new branch..."
    }
)
            try:
                ...
                await add_log(job_id, f"? Pushed migrated code to branch: {branch_url}")
            except Exception as push_error:
                await add_log(job_id, f"?? Branch push failed: {str(push_error)}")
                await add_log(job_id, f"?? Migrated code saved locally at: {clone_path}")
            ...
            await add_log(job_id, f"Target repository name: {target_repo_name}")
            await update_job(job_id, MigrationStatus.PUSHING, 90, "Creating new repository and pushing migrated code...")
            await manager.broadcast(
    job_id,
    {
        "type": "pushing",
        "progress": 90,
        "status": "PUSHING",
        "message": "Creating new repository and pushing migrated code..."
    }
)
            try:
    ...
    job_id,
    {
        ...
        "progress": 95,
        "status": "PUSHED",
        "message": "Migrated code pushed successfully!"
    }
)
                await add_log(job_id, f"? Created new repository: {new_repo_url}")
            except Exception as push_error:
                await add_log(job_id, f"?? GitHub push failed: {str(push_error)}")
                await add_log(job_id, f"?? Migrated code saved locally at: {clone_path}")
        ...
        if request.email and request.email.strip():
            ...
            if success:
                await add_log(job_id, f"Migration summary sent to {request.email}")
            else:
                await add_log(job_id, f"Failed to send migration summary to {request.email}")
        
        # Complete
        await update_job(job_id, MigrationStatus.COMPLETED, 100, "Migration completed successfully!")
    ...
    job_id,
    {
        ...
        # Clean up message buffer now that job is done
        await manager.finalize_job(job_id)
        ...
        await add_log(job_id, f"ERROR: {str(e)}")
        ...
        # Clean up message buffer now that job failed
        await manager.finalize_job(job_id)
...
async def update_job(
    ...
    # Push progress to all connected WebSocket clients
    await manager.broadcast(
        job_id,
        {
            "type": "progress",
    ...
    # Also store and broadcast the log
    await add_log(job_id, step)
...
async def add_log(job_id: str, message: str):
    ...
    # Store log
    migration_jobs[job_id].migration_log.append(log)

    # Broadcast log
    await manager.broadcast(job_id, log)

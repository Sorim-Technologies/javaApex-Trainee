...
const AppShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  useEffect(() => {
    const readStoredUser = () => {
      try {
        ...
        if (session) {
          setAuthSession(JSON.parse(session));
          return;
        }

        const rawGitHubUser = window.localStorage.getItem("github_user");
        if (rawGitHubUser) {
          setAuthSession({
            provider: "github",
            user: JSON.parse(rawGitHubUser),
          });
          return;
        }

        const rawGoogleUser = window.localStorage.getItem("google_user");
        if (rawGoogleUser) {
          setAuthSession({
            provider: "google",
            user: JSON.parse(rawGoogleUser),
          });
          return;
        }
    ...
    readStoredUser();
    window.addEventListener("auth-changed", readStoredUser);
    window.addEventListener("github-auth-changed", readStoredUser);
    window.addEventListener("focus", readStoredUser);

    return () => {
      window.removeEventListener("auth-changed", readStoredUser);
      window.removeEventListener("github-auth-changed", readStoredUser);
  ...
  const startGoogleLogin = () => {
    ...
    if (!googleConfigured) {
      window.alert(
        "Google login is not configured on the server yet. Add GOOGLE_CLIENT_ID and GOOGLE_CLIENT_SECRET to enable it.",
      );
      return;
    }
    ...
    <div style={shellStyles.root}>
      <header style={shellStyles.header}>
        <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
          ...
          />
          <span style={{ fontWeight: 900, color: "#1e293b", fontSize: 20 }}>
            {" "}
            Full Migration
          </span>
        </div>
        <nav style={{ display: "flex", alignItems: "center", gap: 8 }}>
          ...
          <div style={{ position: "relative" }}>
            <button
              ...
              title="Profile"
            >
              {authSession?.user?.avatar_url ? (
                <img
                  src={authSession.user.avatar_url}
                  alt={
                    authSession.user.login || authSession.user.name || "User"
                  }
                  style={{
                    width: 28,
                    height: 28,
              ...
              <>
                ...
                >
                  ...
                  >
                    ...
                    >
                      {authSession?.user?.name || authSession?.user?.login
                        ? `Welcome, ${authSession.user.name || authSession.user.login}`
                        : "Welcome"}
                    </div>
                    <div
                    ...
                    >
                      {authSession?.provider
                        ? `Signed in with ${authSession.provider === "google" ? "Google" : "GitHub"}`
                        : "Sign in to continue"}
                    </div>
                  </div>
                        ...
                        <div style={{ display: "flex", gap: 8, marginTop: 12 }}>
                          <button
                            ...
                            style={{
                              ...
                              borderRadius: 8,
                              border: "1px solid #e2e8f0",
                              background: googleConfigured ? "#fff" : "#f1f5f9",
                              color: googleConfigured ? "#1e293b" : "#94a3b8",
                              fontSize: 13,
                              fontWeight: 500,
                              cursor: "pointer",
                              opacity: googleConfigured ? 1 : 0.75,
                              transition: "all 0.2s ease",
                            }}
                          ...
                          >
                            <svg
                              ...
                              height="16"
                              viewBox="0 0 24 24"
                              style={{ opacity: googleConfigured ? 1 : 0.5 }}
                            >
                              <path
                                fill="#9ca3af"
      ...
      <footer style={shellStyles.footer}>
        <div style={shellStyles.footerContent}>
          ...
          <span>
            ...
            <a href="https://sorim.ai/">Sorim.ai</a>
          </span>
        </div>
      </footer>
    </div>

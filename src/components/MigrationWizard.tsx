...
interface PersistedWizardFormState {
  ...
  createRepoDestinationPlatform?: string;
...
const MIGRATION_STEPS = [
  {
    ...
    name: "Connect",
    icon: "🔗",
    description: "Connect to GitHub or GitLab Repository",
    summary: "Enter your GitHub or GitLab repository URL to start the migration process"
  },
  {
    id: 2,
    name: "Discovery",
...
export default function MigrationWizard({ onBackToHome }: { onBackToHome?: () => void }) {
  ...
  const extractRepoOwner = (url: string) => {
    ...
    try {
      ...
      const parts = raw
        .replace(/^https?:\/\//i, "")
        .replace(/^(github\.com|gitlab\.com)\//i, "")
        .replace(/\.git$/i, "")
        .split("/")
        .filter(Boolean);
      return parts[0] || "username";
  ...
  const sanitizeTargetRepoName = (value: string) => {
    ...
    return lastPart.replace(/\.git$/i, "").trim();
  };

  const buildTargetRepoName = (repoName: string, timestamp: string) =>
    `${repoName || "repo"}-Migrated${timestamp}`;

  const getStoredGithubUsername = () => {
    if (typeof window === "undefined") return "";
    try {
      const storedUser = window.localStorage.getItem("github_user");
      if (!storedUser) return "";
      const parsedUser = JSON.parse(storedUser);
      return String(parsedUser?.login || parsedUser?.username || parsedUser?.name || "").trim();
    } catch {
      return "";
    }
  };

  const buildTargetRepoPrefix = (platform: string = "github", owner: string = "username") =>
    platform === "gitlab"
      ? `https://gitlab.com/${owner || "username"}/`
      : `https://github.com/${owner || "username"}/`;

  const buildTargetRepoUrl = (repoName: string, timestamp: string, platform: string = "github", owner: string = "username") =>
    `${buildTargetRepoPrefix(platform, owner)}${buildTargetRepoName(repoName, timestamp)}`;
  ...
  const [createRepoDestinationPlatform, setCreateRepoDestinationPlatform] = useState(
    persistedFormState?.createRepoDestinationPlatform ?? ""
  );
  ...
  // Existing Repository Feature States
  const [destinationRepoUrl, setDestinationRepoUrl] = useState("");
  ...
  const [destAccessChecked, setDestAccessChecked] = useState(false);
  const [destAccessChecking, setDestAccessChecking] = useState(false);
  const [destAccessError, setDestAccessError] = useState("");

  const effectiveCreateRepoDestinationPlatform = createRepoDestinationPlatform || detectPlatform(selectedRepo?.url || repoUrl);
  const [createRepoTokenOwners, setCreateRepoTokenOwners] = useState<Record<string, string>>({});
  const [createRepoOwnerLoading, setCreateRepoOwnerLoading] = useState(false);
  const [createRepoOwnerError, setCreateRepoOwnerError] = useState("");
  const targetRepoOwner = createRepoTokenOwners[effectiveCreateRepoDestinationPlatform] || "username";
  const targetRepoPrefix = buildTargetRepoPrefix(effectiveCreateRepoDestinationPlatform, targetRepoOwner);


  useEffect(() => {
    if (migrationApproach !== "fork") return;

    const platform = effectiveCreateRepoDestinationPlatform as "github" | "gitlab";
    let cancelled = false;

    if (createRepoTokenOwners[platform]) {
      setCreateRepoOwnerError("");
      return;
    }

    setCreateRepoOwnerLoading(true);
    setCreateRepoOwnerError("");

    getPlatformOwner(platform)
      .then((ownerInfo) => {
        if (cancelled) return;
        setCreateRepoTokenOwners((prev) => ({
          ...prev,
          [platform]: ownerInfo.username || "username",
        }));
      })
      .catch((err) => {
        if (cancelled) return;
        setCreateRepoOwnerError(err instanceof Error ? err.message : "Unable to fetch destination token owner");
      })
      .finally(() => {
        if (!cancelled) setCreateRepoOwnerLoading(false);
      });

    return () => {
      cancelled = true;
    };
  }, [migrationApproach, effectiveCreateRepoDestinationPlatform, createRepoTokenOwners]);
  ...
  // Visibility check: detects if the repo is private/public for internal state tracking.
  // Tokens are NOT prompted in the UI — they are read from .env on the backend.
  useEffect(() => {
    if (step !== 1 || !urlValidation.valid) {
    ...
    const timer = setTimeout(() => {
      ...
      getRepoVisibility(normalizedUrl)
        .then((visibility) => {
          if (cancelled) return;
          // Just track visibility state silently — no PAT prompt shown
          setIsPrivateRepo(!!visibility.requires_token);
          setError("");
        })
        .catch(() => {
          // Silently ignore errors — backend will handle auth via .env token
          if (!cancelled) setIsPrivateRepo(false);
        })
        .finally(() => {
  ...
  }, [step, urlValidation.valid, urlValidation.normalizedUrl]);
  ...
  // Auto-fill target repo name when selectedRepo, timestamp, or Create New Repository destination platform changes
  useEffect(() => {
    if (selectedRepo) {
      ...
      setTargetRepoName(
        migrationApproach === "branch"
          ...
          : buildTargetRepoName(sourceRepoName, targetRepoTimestamp)
      );
    }
  }, [selectedRepo, selectedTargetVersion, targetRepoTimestamp, migrationApproach, effectiveCreateRepoDestinationPlatform]);
  ...
  const buildMigrationRequest = () => {
    ...
    const finalDestinationRepoUrl = migrationApproach === "branch" ? (destinationRepoUrl || sourceRepoUrl) : undefined;
    const sourcePlatform = detectPlatform(sourceRepoUrl);
    const destinationPlatform = migrationApproach === "branch"
      ? detectPlatform(finalDestinationRepoUrl || sourceRepoUrl)
      : effectiveCreateRepoDestinationPlatform;
    const finalTargetRepoName = targetRepoName || (
      migrationApproach === "branch"
        ? buildTargetBranchName(repoName, targetRepoTimestamp)
        : buildTargetRepoName(repoName, targetRepoTimestamp)
    );
    const finalCreateRepoName = sanitizeTargetRepoName(finalTargetRepoName) || buildTargetRepoName(repoName, targetRepoTimestamp);

    return {
      source_repo_url: sourceRepoUrl,
      target_repo_name: migrationApproach === "branch" ? finalTargetRepoName : finalCreateRepoName,
      // Keep legacy fields for existing same-platform/GitHub compatibility.
      platform: sourcePlatform,
      token: currentToken,
      // New cross-platform fields. Backend uses these independently.
      source_platform: sourcePlatform,
      destination_platform: destinationPlatform,
      source_token: currentToken,
      destination_token: patToken.trim() || currentToken,
      ...
      destination_repo_url: finalDestinationRepoUrl,
      target_branch_name: migrationApproach === "branch"
        ? (targetBranchName.trim().replace(/^['"`]+|['"`]+$/g, "").trim() || buildTargetBranchName(repoName, targetRepoTimestamp))
        : undefined,
      create_new_branch: migrationApproach === "branch" ? createNewBranch : undefined,
      ...
      <div style={styles.card}>
        <div style={styles.stepHeader}>
          ...
          <div>
            ...
            <p style={styles.subtitle}>Enter a GitHub or GitLab repository URL to start migration analysis.</p>
          </div>
        </div>
    ...
    <div style={styles.card}>
      ...
      {migrationApproach === "branch" ? (
        <>
          ...
          <div style={styles.field}>
            <label style={styles.label}>Destination Repository URL</label>
            <p style={styles.helpText}>
              Enter the URL of the repository to push migrated code into.
              GitHub → GitLab, GitLab → GitHub, and same-platform destinations are supported.
            </p>
            <div style={{ display: "flex", gap: 8 }}>
              <input
                type="text"
                ...
                onChange={(e) => {
                  setDestinationRepoUrl(e.target.value);
                  setDestAccessChecked(false);
                  setDestAccessError("");
                  setAvailableBranches([]);
                }}
                placeholder={selectedRepo?.url || "https://github.com/owner/repo"}
              />
              <button
                style={{
                  ...styles.secondaryBtn,
                  opacity: (!destinationRepoUrl || destAccessChecking) ? 0.5 : 1,
                }}
                onClick={handleCheckDestAccess}
                disabled={!destinationRepoUrl || destAccessChecking}
              >
                {destAccessChecking ? "Checking…" : "Check Access"}
              </button>
            </div>
            {destAccessError && (
              <div style={{ color: "#dc2626", fontSize: 12, marginTop: 6 }}>⚠ {destAccessError}</div>
            )}
            {destAccessChecked && (
              <div style={{ color: "#16a34a", fontSize: 12, marginTop: 6 }}>✓ Access verified — {availableBranches.length} branch{availableBranches.length !== 1 ? "es" : ""} loaded</div>
            )}
          </div>
          ...
          <div style={styles.field}>
            ...
            <div style={{ display: "flex", gap: 24, marginBottom: 12 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 14, cursor: "pointer" }}>
                <input
                  ...
                  onChange={() => { setCreateNewBranch(true); setTargetBranchName(""); }}
                />
                Create new branch
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 14, cursor: "pointer" }}>
            ...
            {createNewBranch ? (
              <div>
                <input
                  type="text"
                  style={{ ...styles.input, backgroundColor: "#f0fdf4", borderColor: "#22c55e" }}
                ...
                />
                <p style={styles.helpText}>
                  Leave blank to auto-generate: <code style={{ backgroundColor: "#f1f5f9", padding: "2px 6px", borderRadius: 4, fontSize: 11 }}>migration/{'{source-repo}'}-Migrated{'{timestamp}'}</code>
                </p>
              </div>
            ) : (
              <div>
                {!destAccessChecked && (
                  <p style={{ ...styles.helpText, color: "#d97706" }}>👆 Click "Check Access" above to load available branches.</p>
                )}
                {destAccessChecked && availableBranches.length === 0 && (
                  <p style={{ ...styles.helpText, color: "#dc2626" }}>No branches found in the destination repository.</p>
                )}
                {availableBranches.length > 0 && (
                  <>
                    ...
                    <p style={styles.helpText}>Migrated code will be pushed to this branch in the destination repository.</p>
                  </>
                )}
              </div>
            )}
        ...
        <>
          <div style={styles.field}>
            <label style={styles.label}>Destination Platform</label>
            <div style={{ display: "flex", gap: 16, marginBottom: 8 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 14, cursor: "pointer" }}>
                <input
                  type="radio"
                  name="createRepoDestinationPlatform"
                  checked={effectiveCreateRepoDestinationPlatform === "github"}
                  onChange={() => setCreateRepoDestinationPlatform("github")}
                />
                GitHub
              </label>
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 14, cursor: "pointer" }}>
                <input
                  type="radio"
                  name="createRepoDestinationPlatform"
                  checked={effectiveCreateRepoDestinationPlatform === "gitlab"}
                  onChange={() => setCreateRepoDestinationPlatform("gitlab")}
                />
                GitLab
              </label>
            </div>
            <p style={styles.helpText}>Select where the new repository should be created.</p>
            {createRepoOwnerLoading && (
              <p style={styles.helpText}>Loading destination account from configured token...</p>
            )}
            {createRepoOwnerError && (
              <p style={{ ...styles.helpText, color: "#dc2626" }}>{createRepoOwnerError}</p>
            )}
          </div>
        <div style={styles.field}>
          ...
          <div style={{ display: "flex", alignItems: "center" }}>
            ...
            <input
              ...
              onChange={(e) => setTargetRepoName(sanitizeTargetRepoName(e.target.value))}
              placeholder={buildTargetRepoName(selectedRepo?.name || "repo", targetRepoTimestamp)}
            />
          </div>
          <p style={styles.helpText}>
            Format: <code style={{ backgroundColor: "#f1f5f9", padding: "2px 6px", borderRadius: 4, fontSize: 11 }}>
              {targetRepoPrefix}{'{source-repo}'}-Migrated{'{timestamp}'}
            </code> (username auto-filled, only repository name is editable)
          </p>
        </div>
        </>
      )}
  ...
  const handleCheckDestAccess = async () => {
    if (!destinationRepoUrl) return;
    setDestAccessChecking(true);
    setDestAccessError("");
    setBranchesLoading(true);

    try {
      // Determine platform from URL
      const isGitLab = destinationRepoUrl.includes("gitlab.com");
      const platform = isGitLab ? "gitlab" : "github";

      await checkRepoAccess(platform, destinationRepoUrl, patToken);
      setDestAccessChecked(true);

      const { branches } = await listRepoBranches(platform, destinationRepoUrl, patToken);
      setAvailableBranches(branches);
    } catch (err: any) {
      setDestAccessError(err.message || "Failed to access destination repository");
      setDestAccessChecked(false);
      setAvailableBranches([]);
    } finally {
      setDestAccessChecking(false);
        ...
        {migrationApproach === "branch" ? (
          <>
            <div style={styles.field}>
              ...
              {destAccessError && <div style={{ color: "red", fontSize: 12, marginTop: 4 }}>{destAccessError}</div>}
              {destAccessChecked && <div style={{ color: "green", fontSize: 12, marginTop: 4 }}>Access verified and branches loaded.</div>}
            </div>
            ...
            <div style={styles.field}>
              ...
              {createNewBranch ? (
                <div>
                  <input
                    type="text"
                    style={{ ...styles.input, width: "100%", backgroundColor: "#f0fdf4", borderColor: "#22c55e" }}
                    value={targetBranchName}
                    onChange={(e) => setTargetBranchName(e.target.value)}
                    placeholder={buildTargetBranchName(selectedRepo?.name || "repo", targetRepoTimestamp)}
                  />
                ...
                <div>
                  ...
                  {branchesLoading && <p style={styles.helpText}>Loading branches...</p>}
                  {!branchesLoading && availableBranches.length === 0 && destAccessChecked && (
                    <p style={styles.helpText}>No branches found or unable to fetch branches.</p>
                  )}
                  {!destAccessChecked && (
                    <p style={styles.helpText}>Please check access to load branches.</p>
                  )}
                </div>
              )}
            </div>

...
const TARGET_REPO_OWNER = "Aalia-sorim";
const LEGACY_TARGET_REPO_OWNER = "SrikkanthSorim";

const normalizeTargetRepoOwner = (value: string) =>
  value.replace(new RegExp(`github\\.com/`, "gi"), `github.com/`);
...
export default function MigrationWizard({ onBackToHome }: { onBackToHome?: () => void }) {
  ...
  const buildTargetRepoUrl = (repoName: string, timestamp: string) =>
    `${`https://github.com/${TARGET_REPO_OWNER}`}/${repoName || "repo"}-Migrated${timestamp}`;
  ...
  const [targetRepoName, setTargetRepoName] = useState(() =>
    normalizeTargetRepoOwner(persistedFormState?.targetRepoName ?? "")
  );
  ...
  const hasMigrationAnalysisTool = runSonar || runFossa;
  ...
  const renderStep1 = () => {
    ...
    const isRepositoryConnected = Boolean(selectedRepo) || urlValidation.valid;
    const connectionState = isRepositoryConnected ? "Connected" : "Not connected";
        ...
        <p style={styles.helpText}>
          Format: <code style={{ backgroundColor: "#f1f5f9", padding: "2px 6px", borderRadius: 4, fontSize: 11 }}>
            {migrationApproach === "branch"
              ...
              : <>https://github.com/Aalia-sorim/{'{source-repo}'}-Migrated{'{timestamp}'}</>}
  ...
  const renderMigrationStep = () => {
    ...
    <div style={styles.card}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(280px, 1fr))", gap: 16 }}>
          {[
            ...
            >
              ...
              <div style={{ display: "flex", alignItems: "flex-start", gap: 14, marginBottom: 12, paddingRight: item.showInfo ? 28 : 0 }}>
                <div style={{
                  width: 48,
                  height: 48,
                  borderRadius: 12,
      ...
      <div style={styles.field}>
        ...
        <div style={{ marginTop: 12, fontSize: 13, color: hasMigrationAnalysisTool ? "#047857" : "#b45309", fontWeight: 600 }}>
          To start migration: click FOSSA or SonarQube to proceed.
        </div>
      </div>
      ...
      <div style={styles.btnRow}>
        <button style={styles.secondaryBtn} onClick={() => setStep(3)}>← Back</button>
        <button
          style={{ ...styles.primaryBtn, opacity: loading || !hasMigrationAnalysisTool ? 0.5 : 1 }}
          onClick={handleStartMigration}
          disabled={loading || !hasMigrationAnalysisTool}
        >

...
export default function MigrationWizard({ onBackToHome }: { onBackToHome?: () => void }) {
  ...
  const enrichAnalysisWithPomVersion = async (analysis: RepoAnalysis, repoUrlToAnalyze: string, token: string) => {
    ...
    // Backend already parsed pom.xml — only fetch again if truly missing
    const needsPomFallback =
      ...
      !analysis.java_version_detected_from_build;
  ...
  const applyRepositoryAnalysis = (analysis: RepoAnalysis) => {
    ...
    // Populate root file listing from analysis (backend pre-caches it — zero extra API call)
    if (Array.isArray((analysis as any).root_files) && (analysis as any).root_files.length > 0) {
      setRepoFiles((analysis as any).root_files);
    }
  ...
  useEffect(() => {
    if (step === 2 && selectedRepo && currentPath === "") {
      // Root listing is pre-cached by analyze_repository — only fetch for subdirectory navigation
      return;
    }
    if (step === 2 && selectedRepo && currentPath !== "") {

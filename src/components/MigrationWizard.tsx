...
const styles: { [key: string]: React.CSSProperties } = {
  ...
  stepDotActive: { width: 12, height: 12, borderRadius: "50%", background: "var(--primary)", boxShadow: "0 0 0 6px rgba(37, 99, 235, 0.2)" },
  stepDotCompleted: { width: 12, height: 12, borderRadius: "50%", background: "var(--success-text)", boxShadow: "0 0 0 6px rgba(16, 185, 129, 0.2)" },
  stepLabel: { fontSize: 12, fontWeight: 500, color: "var(--muted)" },
  ...
  warning: { background: "var(--warning-bg)", border: "1px solid var(--warning-border)", borderRadius: 8, padding: "12px 16px", color: "var(--warning-text)", fontSize: 14 },
  ...
  sectionTitle: { fontSize: "clamp(16px, 2vw, 20px)", fontWeight: 700, color: "var(--text)", marginBottom: 16, display: "flex", alignItems: "center", gap: 10, wordWrap: "break-word" },
  helpText: { fontSize: 13, color: "var(--muted)", marginTop: 6, lineHeight: 1.5, wordWrap: "break-word" },
  infoBox: { padding: 16, background: "var(--surface-alt)", border: "1px solid var(--border)", borderRadius: 8, color: "var(--text)", fontSize: 14, lineHeight: 1.6 },
  warningBox: { background: "var(--warning-bg)", border: "1px solid var(--warning-border)", borderRadius: 8, padding: "14px 16px", marginBottom: 20, color: "var(--warning-text)" },
  warningTitle: { fontWeight: 600, marginBottom: 10, color: "var(--warning-text)", fontSize: 14 },
  timerBadge: { display: "inline-flex", flexDirection: "column", alignItems: "center", padding: "8px 16px", background: "linear-gradient(135deg, #667eea 0%, #764ba2 100%)", borderRadius: 12, color: "white", boxShadow: "0 4px 12px rgba(99, 102, 241, 0.3)" },
  timerLabel: { fontSize: 10, fontWeight: 600, textTransform: "uppercase", letterSpacing: "0.5px", opacity: 0.9 },
  timerValue: { fontSize: 18, fontWeight: 700, fontFamily: "'JetBrains Mono', monospace" },
...
export default function MigrationWizard({ onBackToHome }: { onBackToHome?: () => void }) {
  ...
  const handleRepositoryContinue = async () => {
    ...
    const repoName = normalizedUrl.split("/").pop()?.replace(/\.git$/, "") || "repository";
    ...
    setSelectedRepo({
      name: repoName,
      full_name: normalizedUrl.replace(/^https?:\/\/[^/]+\//, "").replace(/\.git$/, ""),
      url: normalizedUrl,
      default_branch: "main",
      language: null,
      description: null,

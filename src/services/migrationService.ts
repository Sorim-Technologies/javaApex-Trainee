...
async function parseJsonResponse<T>(response: Response, fallbackMessage: string): Promise<T> {
  ...
  if (!contentType.includes("application/json")) {
    if (bodyText.trim().startsWith("<!doctype") || bodyText.trim().startsWith("<html")) {
      const err = new Error(`API routing error: expected JSON from ${response.url} but received HTML. Check VITE_API_URL or backend routing.`);
...
export async function fetchRepositories(token: string): Promise<RepoInfo[]> {
  ...
  if (!response.ok) {
    ...
    const err = new Error(error.detail || 'Failed to fetch repositories');
    (err as any).status = response.status;
    throw err;
  }
...
export async function analyzeRepository(token: string, owner: string, repo: string): Promise<RepoAnalysis> {
  ...
  if (!response.ok) {
    const error = await response.json();
    const err = new Error(error.detail || 'Failed to analyze repository');
    (err as any).status = response.status;
    throw err;
  }
  return response.json();
...
export async function fetchDependencyGraph(
  ...
  const response = await fetch(
    `${API_BASE_URL}/analysis/dependency-graph?repo_url=${encodeURIComponent(
      repoUrl
    )}&token=${encodeURIComponent(token)}`
  );

  return parseJsonResponse<DependencyGraphResponse>(
    response,
...
export interface CodeComplexityResponse {
  summary: ComplexitySummary;
  files: ComplexityFile[];
}

export async function getCodeComplexity(
  repoUrl: string,
  token: string = ""
): Promise<CodeComplexityResponse> {

  const response = await fetch(
    `${API_BASE_URL}/analysis/code-complexity?repo_url=${encodeURIComponent(repoUrl)}&token=${encodeURIComponent(token)}`
  );

  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.detail || "Failed to analyze code complexity");
  }

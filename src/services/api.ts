...
// Automatically routes to GitLab or GitHub endpoint based on URL
export async function analyzeRepoUrl(repoUrl: string, token: string = ""): Promise<RepoUrlAnalysis> {
  const platform = repoUrl.includes("gitlab.com") ? "gitlab" : "github";
  const response = await fetch(
    `${API_BASE_URL}/${platform}/analyze-url?repo_url=${encodeURIComponent(repoUrl)}&token=${encodeURIComponent(token)}`
  );
...
export async function getRepoVisibility(repoUrl: string, token: string = ""): Promise<RepoVisibilityInfo> {
  const platform = repoUrl.includes("gitlab.com") ? "gitlab" : "github";
  // GitLab uses its own visibility endpoint
  const endpoint = platform === "gitlab"
    ? `${API_BASE_URL}/gitlab/repo-visibility?repo_url=${encodeURIComponent(repoUrl)}&token=${encodeURIComponent(token)}`
...
// Automatically routes to GitLab or GitHub based on the repo URL
export async function listRepoFiles(repoUrl: string, token: string = "", path: string = ""): Promise<RepoFilesResponse> {
  const platform = repoUrl.includes("gitlab.com") ? "gitlab" : "github";
  const response = await fetch(
    `${API_BASE_URL}/${platform}/list-files?repo_url=${encodeURIComponent(repoUrl)}&token=${encodeURIComponent(token)}&path=${encodeURIComponent(path)}`
  );
  if (!response.ok) {
    const error = await response.json();
...
export async function getFileContent(repoUrl: string, filePath: string, token: string = ""): Promise<FileContentResponse> {
  const platform = repoUrl.includes("gitlab.com") ? "gitlab" : "github";
  const response = await fetch(
    `${API_BASE_URL}/${platform}/file-content?repo_url=${encodeURIComponent(repoUrl)}&file_path=${encodeURIComponent(filePath)}&token=${encodeURIComponent(token)}`
  );
  if (!response.ok) {
    const error = await response.json();
...
export async function checkRepoAccess(platform: string, repoUrl: string, token: string = ""): Promise<{ success: boolean; message: string }> {
  const p = platform.toLowerCase() === "gitlab" ? "gitlab" : "github";
  const response = await fetch(
    `${API_BASE_URL}/${p}/check-access?repo_url=${encodeURIComponent(repoUrl)}&token=${encodeURIComponent(token)}`
  );
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.detail || 'Failed to verify repository access');
  }
  return response.json();
}

export async function listRepoBranches(platform: string, repoUrl: string, token: string = ""): Promise<{ branches: string[] }> {
  const p = platform.toLowerCase() === "gitlab" ? "gitlab" : "github";
  const response = await fetch(
    `${API_BASE_URL}/${p}/branches?repo_url=${encodeURIComponent(repoUrl)}&token=${encodeURIComponent(token)}`
  );
  if (!response.ok) {
    const error = await response.json();
    throw new Error(error.detail || 'Failed to list repository branches');
  }
  return response.json();
}
...
export async function getPlatformOwner(platform: "github" | "gitlab"): Promise<PlatformOwnerInfo> {
  const response = await fetch(`${API_BASE_URL}/platform-owner?platform=${encodeURIComponent(platform)}`);
  return parseJsonResponse<PlatformOwnerInfo>(response, `Failed to fetch ${platform} token owner`);
}

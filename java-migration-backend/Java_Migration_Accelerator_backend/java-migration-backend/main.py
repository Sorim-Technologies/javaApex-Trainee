...
# Default GitLab token from environment variable
DEFAULT_GITLAB_TOKEN = os.environ.get("GITLAB_TOKEN", "")
...
class MigrationRequest(BaseModel):
    ...
    # Backward-compatible single-platform fields used by the existing UI/API clients.
    # Keep these so existing GitHub and same-platform flows continue to work unchanged.
    platform: Optional[GitPlatform] = Field(default=None, description="Legacy git platform field")
    token: Optional[str] = Field(default="", description="Legacy personal access token")

    # Cross-platform fields. Source fields are used only for read/clone/input operations;
    # destination fields are used only for access checks, branch operations, and push/create.
    source_platform: Optional[GitPlatform] = Field(default=None, description="Source repository platform")
    destination_platform: Optional[GitPlatform] = Field(default=None, description="Destination repository platform")
    source_token: Optional[str] = Field(default="", description="Token for source repository read/clone operations")
    destination_token: Optional[str] = Field(default="", description="Token for destination repository write operations")
    ...
    destination_repo_url: Optional[str] = Field(default=None, description="URL of the destination repository")
    target_branch_name: Optional[str] = Field(default=None, description="Name of the target branch to push to")
    create_new_branch: Optional[bool] = Field(default=False, description="Whether to create a new branch")
    base_branch: Optional[str] = Field(default=None, description="Base branch if creating a new branch")




def detect_git_platform(repo_url: Optional[str], fallback: Optional[GitPlatform] = None) -> GitPlatform:
    """Detect GitHub/GitLab from a repository URL, with a safe fallback for legacy requests."""
    url = (repo_url or "").lower()
    if "gitlab" in url:
        return GitPlatform.GITLAB
    if "github" in url:
        return GitPlatform.GITHUB
    return fallback or GitPlatform.GITHUB


def get_platform_service(platform: GitPlatform):
    """Return the repository service for the requested platform."""
    return gitlab_service if platform == GitPlatform.GITLAB else github_service


def resolve_platform_token(platform: GitPlatform, explicit_token: Optional[str], legacy_token: Optional[str] = "") -> str:
    """Resolve token with backward-compatible fallback order."""
...
def normalize_target_branch_name(target_branch_name: str, fallback_repo_name: str, timestamp: str) -> str:
    ...
    The UI can send pasted values with quotes or trailing spaces. Git rejects values
    like "'develop '", so always sanitize before checkout/push.
    """
    ...
    # Keep slash support for branch namespaces, but remove characters that commonly
    # come from pasted input and make git check-ref-format fail.
...
@app.get("/api/platform-owner")
async def get_platform_owner(platform: GitPlatform):
    """Return the username/account that owns the configured destination platform token."""
    token = resolve_platform_token(platform, None, None)
    if not token:
        raise HTTPException(status_code=400, detail=f"{platform.value.upper()} token is not configured")
    ...
    try:
        ...
        if response.status_code != 200:
            raise HTTPException(
                ...
                detail=f"Unable to fetch {platform.value} token owner"
            )

        data = response.json()
        username = (data.get("username") or data.get("login") or data.get("name") or "").strip()
        if not username:
            raise HTTPException(status_code=404, detail=f"Unable to determine {platform.value} token owner")

        return {"platform": platform.value, "username": username}
    except HTTPException:
        raise
    except Exception as exc:
        logger.exception("Failed to fetch platform owner")
        raise HTTPException(status_code=500, detail=f"Failed to fetch {platform.value} token owner: {str(exc)}")
...
async def get_gitlab_repo_visibility(repo_url: str, token: str = ""):
    """Check GitLab repository visibility using token (with default fallback)."""
    try:
        owner, repo = await gitlab_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        if not effective_token:
            return {
                ...
                "visibility": "private_or_inaccessible",
                "requires_token": True,
                "message": "A GitLab personal access token is required to check repository visibility."
            }
        repo_info = await gitlab_service.get_repo_info(effective_token, owner, repo)
        return {
            "owner": owner,
            "repo": repo,
            "visibility": "private" if repo_info.get("is_private") else "public",
        ...
        message = str(e)
        normalized = message.lower()
        if "not found" in normalized or "access denied" in normalized or "401" in message or "403" in message:
            try:
                owner, repo = await gitlab_service.parse_repo_url(repo_url)
            except Exception:
                owner, repo = "", ""
            return {
                ...
                "visibility": "private_or_inaccessible",
                "requires_token": True,
                "message": "Repository may be private or token is invalid. Provide a valid GitLab Personal Access Token."
            }
        raise HTTPException(status_code=400, detail=message)
...
async def check_github_repo_access(repo_url: str, token: str = ""):
    try:
        owner, repo = await github_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITHUB_TOKEN
        repo_info = await github_service.get_repo_info(effective_token, owner, repo)
...
async def get_github_branches(repo_url: str, token: str = ""):
    try:
        owner, repo = await github_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITHUB_TOKEN
        branches = await github_service.list_branches(effective_token, owner, repo)
...
async def list_gitlab_repos(token: str = ""):
    """List all repositories accessible with the provided GitLab token (or default)"""
    try:
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        repos = await gitlab_service.list_repositories(effective_token)
        return repos
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def analyze_gitlab_repository(owner: str, repo: str, token: str = ""):
    ...
    try:
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        analysis = await gitlab_service.analyze_repository(effective_token, owner, repo)
        return analysis
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
@app.get("/api/gitlab/check-access")
async def check_gitlab_repo_access(repo_url: str, token: str = ""):
    try:
        owner, repo = await gitlab_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        repo_info = await gitlab_service.get_repo_info(effective_token, owner, repo)
...
async def get_gitlab_branches(repo_url: str, token: str = ""):
    try:
        owner, repo = await gitlab_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        branches = await gitlab_service.list_branches(effective_token, owner, repo)
        return {"branches": branches}
    except Exception as e:
        raise HTTPException(status_code=400, detail=str(e))
...
async def list_gitlab_repo_files(repo_url: str, token: str = "", path: str = ""):
    ...
    try:
        owner, repo = await gitlab_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        files = await gitlab_service.list_repo_files(effective_token, owner, repo, path)
        return {
            "repo_url": repo_url,
            "owner": owner,
            "repo": repo,
...
async def get_gitlab_file_content(repo_url: str, file_path: str, token: str = ""):
    ...
    try:
        owner, repo = await gitlab_service.parse_repo_url(repo_url)
        effective_token = token.strip() if token and token.strip() else DEFAULT_GITLAB_TOKEN
        content = await gitlab_service.get_file_content(effective_token, owner, repo, file_path)
        return {
            "repo_url": repo_url,
            "owner": owner,
            "repo": repo,
...
async def analyze_fossa_for_repo(repo_url: str, token: str = ""):
    ...
    """
    try:
        # Choose appropriate source service and token based on the URL.
        repo_platform = detect_git_platform(repo_url)
        repo_service = get_platform_service(repo_platform)
        effective_token = resolve_platform_token(repo_platform, token, "")
...
async def preview_migration_changes(request: MigrationRequest):
    ...
    try:
        ...
        # Preview uses source platform/token only.
        source_platform = request.source_platform or detect_git_platform(request.source_repo_url, request.platform)
        source_service = get_platform_service(source_platform)
        source_token = resolve_platform_token(source_platform, request.source_token, request.token)

        # Clone repository
        clone_path = await source_service.clone_repository(
            source_token,
...
async def run_migration(job_id: str, request: MigrationRequest):
    ...
    try:
        # Resolve source and destination independently.
        # Legacy requests still work because `platform`/`token` remain valid fallbacks.
        source_platform = request.source_platform or detect_git_platform(request.source_repo_url, request.platform)
        destination_repo_url_for_detection = request.destination_repo_url or request.source_repo_url
        destination_platform = request.destination_platform or detect_git_platform(
            destination_repo_url_for_detection,
            request.platform or source_platform,
        )

        source_service = get_platform_service(source_platform)
        destination_service = get_platform_service(destination_platform)
        source_token = resolve_platform_token(source_platform, request.source_token, request.token)
        destination_token = resolve_platform_token(destination_platform, request.destination_token, request.token)

        add_log(job_id, f"Source platform: {source_platform.value}")
        add_log(job_id, f"Destination platform: {destination_platform.value}")

        # Step 1: Clone repository using the source platform only.
        ...
        _, source_repo_name = await source_service.parse_repo_url(request.source_repo_url)
        now = datetime.now().strftime("%Y%m%d-%H%M%S")

        add_log(
            job_id,
            f"Using source {source_platform.value} token: "
            f"{'user-provided' if (request.source_token and request.source_token.strip()) or (request.token and request.token.strip()) else 'default from .env'}"
        )
        add_log(
            job_id,
            f"Using destination {destination_platform.value} token: "
            f"{'user-provided' if (request.destination_token and request.destination_token.strip()) or (request.token and request.token.strip()) else 'default from .env'}"
        )

        migration_approach = (request.migration_approach or "fork").strip().lower()

        if migration_approach == "branch":
            dest_repo_url = request.destination_repo_url if request.destination_repo_url else request.source_repo_url
            target_branch_name = normalize_target_branch_name(
                request.target_branch_name or request.target_repo_name,
                source_repo_name,
                now,
            )
            add_log(job_id, f"Target branch name: {target_branch_name} in {dest_repo_url}")
            update_job(job_id, MigrationStatus.PUSHING, 90, "Pushing migrated code to branch...")
            try:
                branch_url = await destination_service.push_to_branch(
                    destination_token,
                    dest_repo_url,
                    clone_path,
                    target_branch_name,
            ...
            try:
                new_repo_url = await destination_service.create_and_push_repo(
                    destination_token,
                    target_repo_name,
                    clone_path,
                ...
                add_log(job_id, f"?? {destination_platform.value} push failed: {str(push_error)}")
                add_log(job_id, f"?? Migrated code saved locally at: {clone_path}")

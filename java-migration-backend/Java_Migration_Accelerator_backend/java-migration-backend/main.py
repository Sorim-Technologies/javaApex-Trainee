...
async def download_migration_report(job_id: str):
    """Generate and download migration report as HTML with LLM analysis"""
    if job_id not in migration_jobs:
    ...
    # Build compact data dict for LLM
    issues_dicts = [
        {
        ...
        for i in (job.issues or [])
    ]
    deps_dicts = [
        {"group_id": d.group_id, "artifact_id": d.artifact_id,
         "current_version": d.current_version, "new_version": d.new_version, "status": d.status}
        for d in (job.dependencies or [])
    ]
    migration_data = {
        ...
        "sonar_coverage":          job.sonar_coverage,
        ...
        "issues":                  issues_dicts,
        "dependencies":            deps_dicts,
    }

    llm_analysis = await hf_recommendation_service.analyze_migration_results(migration_data)
    html_content = generate_simple_html_report(job, logs, llm_analysis)

    return Response(
        content=html_content,
        media_type="text/html",
        headers={"Content-Disposition": f"attachment; filename=migration-report-{job_id}.html"}
    )
...
def generate_simple_html_report(job: MigrationResult, logs: List[str], llm_analysis: Dict[str, Any] = None) -> str:
    """Professional light-themed HTML report with LLM analysis, FOSSA, SonarQube, issues and deps."""
    import html as _h
    from datetime import datetime, timezone
    ...
    la = llm_analysis or {}
    llm_summary  = esc(la.get("executive_summary", "Analysis not available."))
    llm_risk     = la.get("risk_assessment", "unknown").lower()
    llm_score    = min(100, max(0, int(la.get("migration_quality_score", 0))))
    llm_findings = la.get("key_findings", [])
    llm_recs     = la.get("recommendations", [])

    risk_color = {"low":"#16a34a","medium":"#d97706","high":"#dc2626","critical":"#991b1b"}.get(llm_risk,"#6b7280")
    risk_bg    = {"low":"#f0fdf4","medium":"#fffbeb","high":"#fef2f2","critical":"#fef2f2"}.get(llm_risk,"#f9fafb")
    score_color = "#16a34a" if llm_score >= 80 else ("#d97706" if llm_score >= 60 else "#dc2626")
    circ = 2 * 3.14159 * 42
    offset = circ - (llm_score / 100.0) * circ

    # Metrics
    issues = job.issues or []
    fixed_issues     = [i for i in issues if i.status == IssueStatus.FIXED]
    remaining_issues = [i for i in issues if i.status != IssueStatus.FIXED]
    sonar_passed  = job.sonar_quality_gate and job.sonar_quality_gate.upper() == "PASSED"
    fossa_passed  = job.fossa_policy_status and job.fossa_policy_status.upper() in ("PASSED","COMPLIANT")
    fossa_avail   = bool(job.fossa_policy_status or job.fossa_total_dependencies)
    total_tests   = max(job.api_endpoints_validated or 0, 1)
    passed_tests  = job.api_endpoints_working or 0
    test_rate     = passed_tests / total_tests * 100
    ...
    # Repo links
    def repo_link(url, label):
        if not url: return "N/A"
        if url.startswith("http"): return f'<a href="{esc(url)}" target="_blank">{esc(url)}</a>'
        if url.startswith("local://"): return esc(url.replace("local://","Local: "))
        return esc(url)

    # Conversion badges
    conv_labels = {"java_version":"Java Version Upgrade","maven_to_gradle":"Maven→Gradle",
                   "gradle_to_maven":"Gradle→Maven","javax_to_jakarta":"javax→Jakarta EE",
                   "spring_boot_2_to_3":"Spring Boot 2→3","junit_4_to_5":"JUnit 4→5",
                   "log4j_to_slf4j":"Log4j→SLF4J"}
    conv_html = "".join(f'<span class="badge b-accent">{esc(conv_labels.get(c,c))}</span>' for c in (job.conversion_types or []))

    # Issues table
    issues_rows = ""
    for i in issues[:100]:
        sv = i.severity.value if hasattr(i.severity,'value') else str(i.severity)
        st = i.status.value   if hasattr(i.status,  'value') else str(i.status)
        issues_rows += f"""<tr>
          <td><span class="badge sev-{sv}">{sv.upper()}</span></td>
          <td><span class="badge st-{st.replace('_','-')}">{st.replace('_',' ').upper()}</span></td>
          <td>{esc(i.category)}</td>
          <td style="max-width:300px;word-break:break-word">{esc(i.message)}</td>
          <td style="font-family:monospace;font-size:.8rem;max-width:200px;word-break:break-all">{esc(i.file_path)}{(':'+str(i.line_number)) if i.line_number else ''}</td>
        </tr>"""

    # Deps table
    deps_rows = ""
    for d in (job.dependencies or []):
        cls = {"upgraded":"dep-up","compatible":"dep-ok"}.get(d.status,"dep-warn")
        deps_rows += f"""<tr>
          <td>{esc(d.group_id)}</td><td>{esc(d.artifact_id)}</td>
          <td>{esc(d.current_version)}</td><td>{esc(d.new_version or '—')}</td>
          <td><span class="badge {cls}">{esc(d.status.upper())}</span></td>
        </tr>"""

    # Logs
    log_html = ""
    for ln in logs[-80:]:
        c = "#dc2626" if "ERROR" in ln else ("#d97706" if "WARNING" in ln else ("#16a34a" if ("✅" in ln or "SUCCESS" in ln.upper()) else "#475569"))
        log_html += f'<div style="color:{c};padding:1px 0">{esc(ln)}</div>'

    findings_html = "".join(f"<li>{esc(str(f))}</li>" for f in llm_findings) or "<li>No findings available.</li>"
    recs_html     = "".join(f"<li>{esc(str(r))}</li>" for r in llm_recs)     or "<li>Run full integration tests before deploying.</li>"

    return f"""<!DOCTYPE html>
<html lang="en">
<head>
<meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">
<title>Migration Report — {esc(job.job_id)}</title>
<style>
...
.card{{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:24px;margin-bottom:18px;box-shadow:0 1px 3px rgba(0,0,0,.06)}}
.card h2{{font-size:1.05rem;font-weight:650;color:#0f172a;padding-bottom:12px;margin-bottom:18px;border-bottom:1px solid #e2e8f0;display:flex;align-items:center;gap:8px}}
/* Header */
.hdr{{background:#fff;border:1px solid #e2e8f0;border-radius:10px;padding:32px;margin-bottom:18px;box-shadow:0 1px 3px rgba(0,0,0,.06)}}
.hdr h1{{font-size:1.6rem;font-weight:700}}.hdr .sub{{color:#64748b;margin-top:4px}}.hdr .meta{{color:#94a3b8;font-size:.82rem;margin-top:8px}}
/* Status pill */
.pill{{display:inline-flex;align-items:center;padding:5px 14px;border-radius:20px;font-size:.78rem;font-weight:600;text-transform:uppercase;letter-spacing:.5px}}
.pill-completed{{background:#f0fdf4;color:#16a34a}}.pill-failed{{background:#fef2f2;color:#dc2626}}.pill-running{{background:#fffbeb;color:#d97706}}
/* Metrics */
.metrics{{display:grid;grid-template-columns:repeat(auto-fit,minmax(160px,1fr));gap:14px}}
.metric{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:18px;text-align:center}}
.metric .v{{font-size:1.7rem;font-weight:700}}.metric .l{{font-size:.72rem;color:#94a3b8;text-transform:uppercase;letter-spacing:.5px;margin-top:4px;font-weight:600}}
.green{{color:#16a34a}}.amber{{color:#d97706}}.red{{color:#dc2626}}.blue{{color:#2563eb}}
/* LLM */
.llm-grid{{display:grid;grid-template-columns:160px 1fr;gap:24px;align-items:start}}
.ring-label{{font-size:.78rem;color:#94a3b8;text-align:center;margin-top:6px;font-weight:500}}
.llm-body h3{{font-size:.88rem;font-weight:600;margin:16px 0 8px}}
.llm-body p{{color:#475569;line-height:1.7}}
.llm-body ul{{padding-left:18px;color:#475569}}.llm-body li{{margin-bottom:5px}}
/* Badges */
.badge{{display:inline-block;padding:2px 9px;border-radius:12px;font-size:.7rem;font-weight:600;text-transform:uppercase;letter-spacing:.3px}}
.b-accent{{background:#eff6ff;color:#2563eb}}
.sev-error{{background:#fef2f2;color:#dc2626}}.sev-warning{{background:#fffbeb;color:#d97706}}.sev-info{{background:#f0f9ff;color:#0284c7}}
.st-fixed{{background:#f0fdf4;color:#16a34a}}.st-detected{{background:#fef9c3;color:#a16207}}.st-manual-review{{background:#fce7f3;color:#be185d}}.st-ignored{{background:#f1f5f9;color:#94a3b8}}
.dep-up{{background:#f0fdf4;color:#16a34a}}.dep-ok{{background:#f0f9ff;color:#0284c7}}.dep-warn{{background:#fffbeb;color:#d97706}}
.gate-pass{{background:#f0fdf4;color:#16a34a}}.gate-fail{{background:#fef2f2;color:#dc2626}}
/* Two col */
.two{{display:grid;grid-template-columns:1fr 1fr;gap:18px}}
/* Tables */
.tbl-wrap{{overflow-x:auto}}
table{{width:100%;border-collapse:collapse;font-size:.85rem}}
th{{background:#f8fafc;color:#64748b;font-weight:600;font-size:.72rem;text-transform:uppercase;letter-spacing:.5px;padding:9px 12px;text-align:left;border-bottom:2px solid #e2e8f0}}
td{{padding:9px 12px;border-bottom:1px solid #e2e8f0;color:#475569}}tr:hover td{{background:#f8fafc}}
/* Repo */
.repo-row{{display:flex;align-items:center;gap:12px;padding:12px 16px;background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;margin-bottom:8px}}
.repo-row .rl{{font-size:.72rem;font-weight:600;color:#94a3b8;text-transform:uppercase;min-width:54px}}
.repo-row a{{color:#2563eb;text-decoration:none;word-break:break-all}}.repo-row a:hover{{text-decoration:underline}}
/* Logs */
.logs{{background:#f8fafc;border:1px solid #e2e8f0;border-radius:8px;padding:16px;max-height:380px;overflow-y:auto;font-family:'SF Mono',Monaco,Menlo,monospace;font-size:.8rem;line-height:1.7}}
/* Footer */
.footer{{text-align:center;padding:20px;color:#94a3b8;font-size:.8rem}}
@media(max-width:700px){{.two{{grid-template-columns:1fr}}.llm-grid{{grid-template-columns:1fr}}.metrics{{grid-template-columns:repeat(2,1fr)}}}}
@media print{{body{{background:#fff}}.card,.hdr{{box-shadow:none;break-inside:avoid}}}}
</style>
</head>
<body>
<div class="wrap">
...
<div class="hdr">
  <div style="display:flex;justify-content:space-between;align-items:flex-start;flex-wrap:wrap;gap:12px">
    <div>
      ...
      <div class="sub">Java {esc(job.source_java_version)} &rarr; Java {esc(job.target_java_version)}</div>
      <div class="meta">Job: {esc(job.job_id)} &bull; Generated: {now} &bull; Duration: {duration}</div>
      <div style="margin-top:10px">{conv_html}</div>
    </div>
    <span class="pill pill-{status_val.lower()}">{status_val.upper()}</span>
  </div>
</div>
...
<div class="card">
  ...
  <div class="metrics">
    <div class="metric"><div class="v blue">{job.files_modified}</div><div class="l">Files Modified</div></div>
    <div class="metric"><div class="v">{len(issues)}</div><div class="l">Total Issues</div></div>
    <div class="metric"><div class="v green">{len(fixed_issues)}</div><div class="l">Issues Fixed</div></div>
    <div class="metric"><div class="v {'red' if remaining_issues else 'green'}">{len(remaining_issues)}</div><div class="l">Remaining</div></div>
    <div class="metric"><div class="v">{job.errors_fixed}/{job.total_errors}</div><div class="l">Errors Fixed</div></div>
    <div class="metric"><div class="v">{job.warnings_fixed}/{job.total_warnings}</div><div class="l">Warnings Fixed</div></div>
  </div>
</div>
  ...
  <div class="llm-grid">
    <div>
      <svg viewBox="0 0 100 100" width="120" height="120">
        ...
        <text x="50" y="46" text-anchor="middle" font-size="22" font-weight="700" fill="{score_color}">{llm_score}</text>
        <text x="50" y="60" text-anchor="middle" font-size="8" fill="#94a3b8">/ 100</text>
      </svg>
      <div class="ring-label">Quality Score</div>
      <div style="text-align:center;margin-top:10px">
        <span class="badge" style="background:{risk_bg};color:{risk_color}">{llm_risk.upper()} RISK</span>
      </div>
    </div>
    <div class="llm-body">
      <p>{llm_summary}</p>
      <h3>Key Findings</h3><ul>{findings_html}</ul>
...
<div class="two">
  <div class="card">
    <h2>&#x1F9EA; Test Results</h2>
    <div class="metrics">
      <div class="metric"><div class="v">{total_tests}</div><div class="l">Total</div></div>
      <div class="metric"><div class="v green">{passed_tests}</div><div class="l">Passed</div></div>
      <div class="metric"><div class="v red">{total_tests-passed_tests}</div><div class="l">Failed</div></div>
      <div class="metric"><div class="v {'green' if test_rate>=80 else 'red'}">{test_rate:.1f}%</div><div class="l">Rate</div></div>
    </div>
  </div>
  <div class="card">
    <h2>&#x1F50D; SonarQube</h2>
    <div style="margin-bottom:14px">
      <span class="pill {'gate-pass' if sonar_passed else 'gate-fail'}">{'&#x2705; PASSED' if sonar_passed else ('&#x274C; '+(esc(job.sonar_quality_gate or 'Not Run')))}</span>
    </div>
    <div class="metrics">
      <div class="metric"><div class="v red">{job.sonar_bugs}</div><div class="l">Bugs</div></div>
      <div class="metric"><div class="v amber">{job.sonar_vulnerabilities}</div><div class="l">Vulns</div></div>
      <div class="metric"><div class="v blue">{job.sonar_code_smells}</div><div class="l">Smells</div></div>
      <div class="metric"><div class="v">{job.sonar_coverage:.1f}%</div><div class="l">Coverage</div></div>
    </div>
  </div>
</div>
...
<!-- FOSSA -->
<div class="card">
  <h2>&#x1F6E1; FOSSA License &amp; Dependency Compliance</h2>
  {'<div style="margin-bottom:14px"><span class="pill '+ ("gate-pass" if fossa_passed else "gate-fail") +'">'+("&#x2705; COMPLIANT" if fossa_passed else "&#x274C; "+(esc(str(job.fossa_policy_status or "NOT RUN"))))+'</span></div><div class="metrics"><div class="metric"><div class="v blue">'+str(job.fossa_total_dependencies)+'</div><div class="l">Dependencies</div></div><div class="metric"><div class="v amber">'+str(job.fossa_license_issues)+'</div><div class="l">License Issues</div></div><div class="metric"><div class="v red">'+str(job.fossa_vulnerabilities)+'</div><div class="l">Vulnerabilities</div></div><div class="metric"><div class="v">'+str(job.fossa_outdated_dependencies)+'</div><div class="l">Outdated</div></div></div>' if fossa_avail else '<p style="color:#94a3b8">FOSSA was not run for this migration. Set <code>run_fossa: true</code> to enable compliance scanning.</p>'}
</div>

<!-- Issues -->
<div class="card">
  <h2>&#x26A0; Issues ({len(issues)} total)</h2>
  {'<div class="tbl-wrap"><table><thead><tr><th>Severity</th><th>Status</th><th>Category</th><th>Message</th><th>File</th></tr></thead><tbody>'+issues_rows+'</tbody></table></div>' if issues_rows else '<p style="color:#94a3b8">No issues detected.</p>'}
</div>

<!-- Dependencies -->
<div class="card">
  <h2>&#x1F4E6; Dependencies ({len(job.dependencies or [])} tracked)</h2>
  {'<div class="tbl-wrap"><table><thead><tr><th>Group</th><th>Artifact</th><th>Current</th><th>New</th><th>Status</th></tr></thead><tbody>'+deps_rows+'</tbody></table></div>' if deps_rows else '<p style="color:#94a3b8">No dependency information available.</p>'}
</div>

<!-- Repos -->
<div class="card">
  <h2>&#x1F517; Repositories</h2>
  <div class="repo-row"><span class="rl">Source</span>{repo_link(job.source_repo,'source')}</div>
  <div class="repo-row"><span class="rl">Target</span>{repo_link(job.target_repo,'target') if job.target_repo else '<span style="color:#94a3b8">Not available</span>'}</div>
</div>

<!-- Logs -->
<div class="card">
  <h2>&#x1F4DD; Migration Logs</h2>
  <div class="logs">{log_html or '<span style="color:#94a3b8">No log entries.</span>'}</div>
</div>

<div class="footer">Java Migration Accelerator &mdash; {now}</div>
</div>
</body>
</html>"""

...
MIGRATION_STATUS_FILTERS = {"running", "completed", "failed"}


def normalize_migration_status_filter(value: Optional[str]) -> Optional[str]:
    value = (value or "").strip().lower()
    if not value or value == "all":
        return None
    return value if value in MIGRATION_STATUS_FILTERS else "__invalid_status__"
...
def get_migration_logs(
    ...
    status_filter = normalize_migration_status_filter(status)

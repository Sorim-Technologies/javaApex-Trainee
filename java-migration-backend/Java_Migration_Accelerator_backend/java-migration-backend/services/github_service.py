...
# ---------------------------------------------------------------------------
# Shared, pooled GitHub clients.
#
# Previously every call created a brand new Github()/requests session, which
# meant a fresh TCP+TLS handshake to api.github.com on every single API call
# (visibility check, analyze, file listing, file content all paid this cost
# independently). We now keep one pooled, keep-alive session per
# (token, base_url) pair and reuse the underlying PyGithub client across
# calls for the lifetime of the process. This alone removes one full
# connection setup per request after the first.
# ---------------------------------------------------------------------------
_client_cache: Dict[tuple, Github] = {}
_client_cache_lock = threading.Lock()


def _build_pooled_session() -> requests.Session:
    """A requests.Session with connection pooling + automatic retry/backoff
    for transient errors (timeouts, 5xx, secondary rate limits), so a single
    flaky round trip doesn't surface as a multi-second hang to the user."""
...
def get_github_client(token: str, repo_url: str = None) -> Github:
    """Return a pooled, reused Github client for public or enterprise GitHub.

    Clients are cached per (token, base_url) so repeated calls (visibility
    check -> analyze -> file content, etc.) reuse the same keep-alive HTTP
    connection pool instead of paying a fresh TLS handshake every time.
    """
    token = token.strip() if token else ""
    base_url = "https://api.github.com"

    if repo_url and "github.com" in repo_url and "github." not in repo_url.replace("github.com", ""):
        pass  # public GitHub, default base_url is correct
    elif repo_url:
    ...
    with _client_cache_lock:
        ...
        kwargs: Dict[str, Any] = {"timeout": 15, "pool_size": 20}
        ...
        try:
            import inspect
            init_params = inspect.signature(Github.__init__).parameters
            if "session" in init_params:
                client = Github(**kwargs, session=session)
        except Exception:
            client = None

        if client is None:
            # Fall back to PyGithub's own connection pooling (it pools
            # internally via urllib3 even without a custom session) - still
            # far better than constructing a brand new client per call.
...
class GitHubService:
    ...
    async def analyze_repository(
        ...
        try:
            ...
            # Populate the repo_info cache too, so a visibility check
            # (get_repo_info) right before or after this call doesn't pay
            # for another fresh GitHub round trip for data we already have.
            set_cached(f"repo_info:{owner}/{repo}", {
            ...
            try:
                ...
                root_names = {p.split("/")[0] for p in all_paths} if all_paths else set()

                # If tree failed, fall back to single root listing only
                if not all_paths:
                    try:
                        root_contents = repository.get_contents("")
                        root_names = {c.name for c in root_contents}
                        # Store root files for list_repo_files reuse
                        root_files_cache = [
                            {"name": c.name, "path": c.path,
                             "type": "dir" if c.type == "dir" else "file",
                             "size": getattr(c, "size", 0), "url": c.html_url}
                            for c in root_contents
                        ]
                        set_cached(f"files:{owner}/{repo}:", root_files_cache)
                    except Exception:
                        root_names = set()
                else:
                    # Build root file listing from tree (zero extra API calls)
                    root_entries = {}
                    for p in all_paths:
                        top = p.split("/")[0]
                        if top not in root_entries:
                            is_dir = any(x.startswith(top + "/") for x in all_paths if x != top)
                            root_entries[top] = {
                                "name": top, "path": top,
                                "type": "dir" if is_dir else "file",
                                "size": 0, "url": f"https://github.com/{owner}/{repo}/blob/{repository.default_branch}/{top}"
                            }
                    set_cached(f"files:{owner}/{repo}:", list(root_entries.values()))

                # ── STEP 2: detect build tool + fetch build file ────────────
                if "pom.xml" in root_names:
                    ...
                    try:
                        ...
                        set_cached(f"file_content:{owner}/{repo}:pom.xml", pom_content)
                        r = self._detect_java_version_from_pom(pom_content)
                        analysis["java_version_from_build"] = r["version"]
                        analysis["java_version_detected_from_build"] = r["detected"]
                        analysis["java_version"] = r["version"] if r["detected"] else "unknown"
                        analysis["dependencies"] = self._parse_pom_dependencies(pom_content)
                    except Exception:
                        analysis["java_version"] = "unknown"
                    is_java_project = True

                elif "build.gradle" in root_names or "build.gradle.kts" in root_names:
                    analysis["build_tool"] = "gradle"
                    analysis["structure"]["has_build_gradle"] = True
                    bf = "build.gradle" if "build.gradle" in root_names else "build.gradle.kts"
                    try:
                        gc = repository.get_contents(bf).decoded_content.decode()
                        r = self._detect_java_version_from_gradle(gc)
                        analysis["java_version_from_build"] = r["version"]
                        analysis["java_version_detected_from_build"] = r["detected"]
                        analysis["java_version"] = r["version"] if r["detected"] else "unknown"
                        analysis["dependencies"] = self._parse_gradle_dependencies(gc)
                    except Exception:
                        analysis["java_version"] = "unknown"
                    is_java_project = True

                elif "build.xml" in root_names:
                    analysis["build_tool"] = "ant"
                    is_java_project = True

                # ── STEP 3: structure + java files from tree (zero API calls) ─
                if any(p == "src/main" or p.startswith("src/main/") for p in all_paths):
                    analysis["structure"]["has_src_main"] = True
                    is_java_project = True
                if any(p == "src/test" or p.startswith("src/test/") for p in all_paths):
                    analysis["structure"]["has_src_test"] = True
                    analysis["has_tests"] = True
                    is_java_project = True

                java_paths = [p for p in all_paths if p.endswith(".java")]
                if java_paths:
                    is_java_project = True
                    analysis["java_files"] = java_paths

                if not is_java_project:
                    if repository.language and repository.language.lower() == "java":
                        is_java_project = True

                if is_java_project and not analysis.get("build_tool"):
                    analysis["build_tool"] = "standalone"

                # No endpoint detection during initial discovery
            ...
            print(f"[ANALYSIS] Done: java={is_java_project}, files={len(analysis.get('java_files',[]))}, build={analysis['build_tool']}")
            return analysis
    ...
    async def get_repo_info(self, token: str, owner: str, repo: str) -> Dict[str, Any]:
        ...
        Cached: this backs the step-1 "is this repo private?" visibility
        check, which previously fired a brand-new, uncached GitHub API call
        on every URL the user typed - duplicating work that analyze_repository
        was about to do again seconds later for the same owner/repo.
        """
        ...
        try:
            g = get_github_client(token.strip() if token else "")
    ...
    async def get_file_content(self, token: str, owner: str, repo: str, path: str) -> str:
        ...
        Cached + uses the shared pooled client: this backs the pom.xml
        fallback fetch that runs right after repository analysis. Without
        caching, re-opening the same wizard step (or retrying after a
        transient error) paid for a full fresh fetch of a file that almost
        never changes mid-session.
        """
        cache_key = f"file_content:{owner}/{repo}:{path}"
        cached = get_cached(cache_key)

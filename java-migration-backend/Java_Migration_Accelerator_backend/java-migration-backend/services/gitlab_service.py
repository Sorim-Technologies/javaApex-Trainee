...
class GitLabService:
    ...
    async def list_branches(self, token: str, owner: str, repo: str) -> List[str]:
        ...
        try:
            ...
            async with httpx.AsyncClient() as client:
                response = await client.get(
                    f"{self.api_base_url}/projects/{project_path.replace('/', '%2F')}/repository/branches",
                    headers=headers
                )
    ...
    async def push_to_branch(self, token: str, repo_url: str, local_path: str, branch_name: str) -> str:
        """Push migrated code to an existing branch in the destination repository.
        
        Strategy: Clone the destination repo, checkout/create the target branch,
        overlay the migrated files on top, then push. This preserves the destination
        repo's commit history and makes files visible in the branch.
        """
        import uuid
        import subprocess
        import re

        branch_name = re.sub(r"\s+", "-", (branch_name or "").strip().strip("'\"`").strip()).replace("\\", "/").strip("/")
        branch_name = re.sub(r"/{2,}", "/", branch_name).strip(".'\"`")
        if not branch_name:
            raise Exception("Target branch name is required.")

        if not token or len(token.strip()) == 0:
            raise Exception("GitLab token is required to push to a branch.")

        # Build authenticated URL for the destination repo
        auth_url = repo_url.replace("https://", f"https://oauth2:{token}@")
        if not auth_url.endswith(".git"):
            auth_url = auth_url + ".git"

        # Clone the DESTINATION repo into a temp directory
        dest_clone_dir = os.path.join(self.work_dir, f"dest_{uuid.uuid4()}")
        os.makedirs(dest_clone_dir, exist_ok=True)

        try:
            print(f"DEBUG push_to_branch: Cloning destination repo {repo_url} ...")
            clone_result = subprocess.run(
                ["git", "clone", auth_url, dest_clone_dir],
                capture_output=True, text=True
            )
            if clone_result.returncode != 0:
                raise Exception(f"Failed to clone destination repo: {clone_result.stderr}")
            print("DEBUG push_to_branch: Destination repo cloned successfully")

            dest_repo = git.Repo(dest_clone_dir)

            # Configure git identity for commits
            with dest_repo.config_writer() as cw:
                cw.set_value("user", "name", "Java Migration Bot")
                cw.set_value("user", "email", "migration@bot.com")

            # Checkout or create the target branch
            remote_branches = [ref.remote_head for ref in dest_repo.remotes.origin.refs]
            print(f"DEBUG push_to_branch: Remote branches: {remote_branches}")
            print(f"DEBUG push_to_branch: Target branch: {branch_name}")

            if branch_name in remote_branches:
                print(f"DEBUG push_to_branch: Checking out existing branch '{branch_name}'")
                dest_repo.git.checkout("-B", branch_name, f"origin/{branch_name}")
            else:
                print(f"DEBUG push_to_branch: Creating new branch '{branch_name}'")
                dest_repo.git.checkout("-b", branch_name)

            # Copy all migrated files from local_path into dest_clone_dir
            print(f"DEBUG push_to_branch: Overlaying migrated files from {local_path} ...")
            for item in os.listdir(local_path):
                # Skip the .git directory of the source
                if item == ".git":
                    continue
                src_item = os.path.join(local_path, item)
                dst_item = os.path.join(dest_clone_dir, item)
            ...
            staged = dest_repo.git.diff("--cached", "--name-only")
            print(f"DEBUG push_to_branch: Staged files:\n{staged}")

            if staged.strip():
                dest_repo.index.commit(
                    "Java migration completed - upgraded Java version, dependencies, and code quality"
                )
                print("DEBUG push_to_branch: Commit created")
            else:
                print("DEBUG push_to_branch: No changes to commit (migrated files identical to destination)")

            # Push to destination repo
            origin = dest_repo.remotes.origin
            origin.set_url(auth_url)
            print(f"DEBUG push_to_branch: Pushing to branch '{branch_name}' ...")
            push_result = origin.push(refspec=f"HEAD:{branch_name}", set_upstream=True, force=True)
            print(f"DEBUG push_to_branch: Push result: {push_result}")
            ...
            # Clean up the temp dest clone
            try:
                shutil.rmtree(dest_clone_dir, ignore_errors=True)
            except Exception:
                pass
